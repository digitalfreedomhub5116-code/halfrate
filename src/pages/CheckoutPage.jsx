...
import { useCartStore, AVAILABLE_COUPONS } from '../store/cartStore'
import {
  getCurrentCustomer,
  getUserAddresses,
  saveUserAddress,
...
export default function CheckoutPage() {
  ...
  // Coupon & Store Financial helpers
  ...
  const shippingFee = typeof getShippingFee === 'function' ? getShippingFee() : (subtotal >= 999 ? 0 : 60)
  const couponDiscount = typeof getCouponDiscount === 'function' ? getCouponDiscount() : 0
  const totalAmount = typeof getFinalTotal === 'function' ? getFinalTotal() : Math.max(0, subtotal + shippingFee - couponDiscount)

  const handleCheckoutApplyCoupon = (codeToApply) => {
    const target = codeToApply || checkoutCouponInput
    const res = applyCoupon(target)
    if (res?.success) {
      setCheckoutCouponSuccess(res.message)
      setCheckoutCouponInput('')
      setTimeout(() => setCheckoutCouponSuccess(''), 2500)
    }
  }
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Clear cart locally and from account & reset applied coupon
      ...
      <main className="mx-auto max-w-2xl px-4 sm:px-6 pt-6">
        ...
        {currentStep === 3 && (
          <div className="space-y-6 animate-fade-in-up">
            ...
            {/* Coupon Code Section */}
            <div className="rounded-2xl border border-[#E7E2D9] bg-white p-4 shadow-sm space-y-2.5">
              <div className="flex items-center justify-between">
              ...
              {appliedCoupon ? (
                ...
                <div>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      placeholder="Coupon Code (e.g. HALFRATE50)"
                      value={checkoutCouponInput}
                      onChange={(e) => setCheckoutCouponInput(e.target.value.toUpperCase())}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') handleCheckoutApplyCoupon()
                      }}
                      className="flex-1 bg-[#FAF8F5] border border-[#E7E2D9] rounded-xl px-3 py-2 text-xs font-mono font-bold text-[#1C1917] placeholder:font-sans placeholder:font-normal uppercase outline-none focus:border-[#991B33]"
                    />
                    <button
                      type="button"
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-1.5">
                    {AVAILABLE_COUPONS.slice(0, 3).map((cp) => (
                      <button
                        key={cp.code}
                        type="button"
                        onClick={() => handleCheckoutApplyCoupon(cp.code)}
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg border border-dashed border-[#991B33]/40 bg-[#FAF8F5] hover:bg-[#FDF2F4] text-[10px] font-bold text-[#991B33] cursor-pointer transition-colors"
                      >
              ...
              <div className="flex justify-between text-[#78716C]">
                ...
                {shippingFee === 0 ? (
                  <span className="font-mono font-bold text-emerald-700">
                    <span className="line-through text-[#78716C]/60 font-normal mr-1.5">₹60</span>
                    FREE
                  </span>
                ) : (
                  <span className="font-mono text-[#1C1917] font-semibold">₹{shippingFee}</span>
