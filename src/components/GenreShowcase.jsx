...
function CategoryCard({ genre, index }) {
  ...
  // Responsive sizes: balanced grid for category count
  const sizeClasses =
    GENRES.length === 4
    ...
    <section id="genres" className="relative py-16 sm:py-24 bg-[#FAF8F5]">
      <div ref={sectionRef} className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div
          className={
            GENRES.length === 4
              ? 'grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-5 auto-rows-[200px] sm:auto-rows-[240px] lg:auto-rows-[260px]'
              : GENRES.length === 3
